...
console.log(" Starting Aureus Africa Telegram Bot with Authentication + Logout...");
 ...
 *For existing account:* Enter your current password
 *For new account:* Create a secure password (min 8 characters)`);
    ...
    // Try to link existing account first
...
Please try again with /register or /login`);
        await updateTelegramUser(ctx.from.id, { registration_step: "start" });
      }
    }
...
async function startBot() {
  try {
    ...
    await bot.launch();
    console.log(" Aureus Africa Bot with Authentication + Logout is running!");
