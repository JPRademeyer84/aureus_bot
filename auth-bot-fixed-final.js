...
console.log(" Starting Aureus Africa Telegram Bot with Fixed Authentication...");
...
bot.command("login", async (ctx) => {
  ...
  await updateTelegramUser(ctx.from.id, { registration_step: "email" });
...
Please enter your existing password to login:

 *Tip:* If you forgot your password, contact support.`);
    } else {
      await ctx.reply(` Email: ${text}
...
Please create a secure password (minimum 8 characters):`);
    ...
    // Check if email exists to determine flow
    const emailExists = await checkEmailExists(updatedUser.temp_email);
...
The email exists but the password is incorrect. Please try again with the correct password, or contact support if you forgot your password.
...
Use /menu to start investing.`);
        ...
        if (createResult.shouldLogin) {
          await ctx.reply(` This email is already registered!

Please use /login instead to access your existing account.`);
        } else {
          await ctx.reply(` ${createResult.message}
...
async function startBot() {
  try {
    ...
    await bot.launch();
    console.log(" Aureus Africa Bot with Fixed Authentication is running!");
