...
async function createNewWebUser(telegramId, email, password, fullName) {
  try {
    ...
    // Auto-increment ID (based on table structure we saw)
    const [result] = await dbConnection.execute(
...
bot.command("menu", requireAuth(async (ctx) => {
  console.log(` Menu command from ${ctx.from.first_name}`);
...
Choose an option below:`;
  ...
  try {
    ...
    await ctx.reply("Sorry, there was an error loading the menu. Please try again.");
  }
}));
...
bot.use(async (ctx, next) => {
  ...
  const user = ctx.from;
  console.log(` Message from ${user.first_name} (@${user.username}): ${ctx.message?.text || ctx.callbackQuery?.data || "non-text"}`);
...
async function startBot() {
  try {
    ...
    await bot.launch();
    console.log(" Aureus Africa Bot FIXED VERSION is running!");
